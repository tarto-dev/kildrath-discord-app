exports.run = (client, message, args) => {
    ...
    getJSON(`https://raider.io/api/v1/characters/profile?region=${region}&realm=${realm}&name=${nickname}&fields=mythic_plus_best_runs`, (err, rsp) => {
        if (typeof rsp.mythic_plus_best_runs == "undefined") {
            message.channel.send("Nickname not found");
            return;
        }

        let runs = rsp.mythic_plus_best_runs;
        ...
        const embed = {
            "title": `Top 3 runs for ${rsp.name} - ${rsp.class} <${rsp.active_spec_name}> (${rsp.active_spec_role})`,
